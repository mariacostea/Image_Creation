function A = bicubic_coef(f, Ix, Iy, Ixy, x1, y1, x2, y2)
    ...
    mat2 = zeros(4, 4);
    mat2(:, 1) = [f(y1, x1) f(y1, x2) Ix(y1, x1) Ix(y1, x2)];
    mat2(:, 2) = [f(y2, x1) f(y2, x2) Ix(y2, x1) Ix(y2, x2)];
    mat2(:, 3) = [Iy(y1, x1) Iy(y1, x2) Ixy(y1, x1) Ixy(y1, x2)];
    mat2(:, 4) = [Iy(y2, x1) Iy(y2, x2) Ixy(y2, x1) Ixy(y2, x2)];

    % TODO: Converte?te matricile intermediare la double.
    
    mat1 = cast(mat1, 'double');
    mat2 = cast(mat2, 'double');
    mat3 = cast(mat3, 'double');
